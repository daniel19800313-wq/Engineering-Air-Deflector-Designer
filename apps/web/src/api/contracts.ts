...
export interface Vector3Input { x: number; y: number; z: number }
...
export interface FanOperatingInput { equipment_id:FanEquipmentId;enabled:boolean;frequency_hz:number }
export interface FanOperatingResult {
  equipment_id:FanEquipmentId;enabled:boolean;status:"OFF"|"STANDBY"|"RUNNING";frequency_hz:EngineeringValue;
  motor_power_hp:EngineeringValue;rated_airflow_cmh:EngineeringValue;rated_static_pressure_pa:EngineeringValue;maximum_frequency_hz:EngineeringValue;
  current_airflow_cmh:EngineeringValue;current_airflow_cfm:EngineeringValue;current_static_pressure_pa:EngineeringValue;inlet_velocity_mps:EngineeringValue;estimation_label:string;
}
