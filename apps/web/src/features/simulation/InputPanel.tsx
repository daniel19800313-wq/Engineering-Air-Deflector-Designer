...
export function InputPanel({form,result,setForm,running,errors,onRun,onReset}:Props){
  const {t}=useI18n();
  const update=(key:keyof WorkspaceForm,value:number|string)=>setForm(current=>({...current,[key]:value}));
  ...
  const selectPreset=(preset:InletDirectionPreset)=>setForm(current=>({...current,inlet_direction_preset:preset,inlet:preset==="custom"?current.inlet:{...current.inlet,direction:PRESET_DIRECTIONS[preset]}}));
  ...
  return <aside className="panel input-panel" aria-labelledby="inputs-title">
    ...
    <fieldset><legend>{t("inletSection")}</legend><div className="field-grid">
      ...
      <div className="vector-group"><span>{t("inletPosition")} <small>m</small></span><div className="vector-fields">{(["x","y","z"] as const).map(axis=><label key={axis}>{axis.toUpperCase()}<input aria-label={`${t("inletPosition")} ${axis.toUpperCase()}`} type="number" step="0.1" value={form.inlet.position[axis]} onChange={event=>updateInlet("position",axis,number(event.target.value))}/></label>)}</div></div>
      <div className="vector-group"><span>{t("inletDirection")} <small>{t("normalizedOnRun")}</small></span><div className="vector-fields">{(["x","y","z"] as const).map(axis=><label key={axis}>{axis.toUpperCase()}<input aria-label={`${t("inletDirection")} ${axis.toUpperCase()}`} type="number" step="0.1" value={form.inlet.direction[axis]} disabled={form.inlet_direction_preset!=="custom"} onChange={event=>updateInlet("direction",axis,number(event.target.value))}/></label>)}</div></div>
